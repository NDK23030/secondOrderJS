...
var accordeon = function accordeon(headSelector) {
  var header = document.querySelectorAll(headSelector);
...
var drop = function drop() {
  // drag *
  // dragend *
  // dragenter - объект перетаскивается над какой-то dropArea
  // dragexit *
  // dragleave - объет за пределами dropArea
  // dragover - объект зависает над dropArea
  // dragstart *
  // drop - обхукт отправлен в drpopArea
  // * - событие срабатывающие на элементах, который мы перетаскиваем (не из файловой системы)
...
var mask = function mask(selector) {
  ...
  inputs.forEach(function (input) {
    input.addEventListener('input', maskNumber); // input.addEventListener('keypress', maskNumber);
...
var modals = function modals(state) {
  var allModals = document.querySelectorAll('[datat-modal]'),
...
var scrolling = function scrolling(upSelector) {
  ...
  //                 hashElementTop += hashElement.offsentTop;
...
var sliders = function sliders(slides, dir, prev, next) {
  var deelay = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 3000;
  var slideIndex = 1,
      paused = false;
